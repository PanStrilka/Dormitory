...
4. Я підготую і дам тобі код функції `parse-receipt` (розбирає фото чека в
   таблицю товарів; якщо ШІ недоступний — лишає фото й запис на кілька днів для
   повторної спроби). Ти вставиш його в **Edge Functions → Deploy a new function**.
