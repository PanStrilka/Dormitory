...
1. **Authentication → Sign In / Providers → Email** → увімкни, і **вимкни
   «Confirm email»** (Confirm email = OFF). Вхід тепер за **email + пароль** —
   без листів. Це навмисно: на iPhone (додаток на екрані «Домівки») лінк із
   пошти відкривається в Safari (окремі cookies), тому пошта там не працює;
   а стандартна пошта Supabase ще й обмежена ~2–4 листами/годину. Пароль усе
   вирішує — сесія створюється прямо в додатку.
2. **Authentication → URL Configuration**:
   - **Site URL:** `https://panstrilka.github.io/Dormitory/`
   - Додай цю ж адресу в **Redirect URLs**.
   - *(SMTP і шаблони листів не потрібні — вхід за паролем їх не використовує.)*
...
## 📷 Етап 6 — Фото-доказ прибирання + ШІ-перевірка

Тепер, перш ніж відмітити завдання виконаним, людина фотографує його **прямо в
застосунку** (без галереї — тільки жива камера), а ШІ перевіряє **дві речі**:
чи завдання справді виконане, і чи це **не фотографія екрана** (захист від
шахрайства). Фото не зберігається на сервері — у застосунку лишається лише
маленька мініатюра як доказ.

1. Розгорни функцію **[`supabase/functions/verify-task/index.ts`](../supabase/functions/verify-task/index.ts)**
   (**Edge Functions → Deploy a new function**, назва рівно `verify-task`).
   Секрет `ANTHROPIC_API_KEY` (і за бажанням `ANTHROPIC_MODEL=claude-haiku-4-5`
   для дешевшого варіанта) — ті самі, що вже додані для `parse-receipt`.
2. У застосунку: **Профіль → Наставення → Фото-důkaz úklidu** — обери, коли
   вимагати фото: *вимкнено* / *тижневі та місячні завдання* / *усі завдання*.

> Працює і без ШІ: якщо Supabase не під’єднано, фото все одно робиться в
> застосунку й зберігається як доказ — просто без автоматичної перевірки.
> Якщо в пристрої немає камери, завдання відмічається як раніше (без фото).

---

## 🗑 Етап 7 — Видалення власного акаунта (необов'язково)

Кнопка **«Smazat účet»** у Профіль → Наставення → Účet дозволяє людині повністю
видалити свій акаунт (членство + профіль + сам обліковий запис).

1. Розгорни функцію **[`supabase/functions/delete-account/index.ts`](../supabase/functions/delete-account/index.ts)**
   (**Edge Functions → Deploy a new function**, назва рівно `delete-account`).
   Додаткових секретів не треба — `SUPABASE_SERVICE_ROLE_KEY` уже є автоматично.

> Якщо цю функцію не розгорнути, кнопка все одно працює «частково»: людину
> **виженуть із буньки** (заявку/членство видалено, доступ втрачено), але сам
> обліковий запис у Supabase лишиться. Для повного видалення потрібна функція.
